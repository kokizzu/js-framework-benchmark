...
const add = () => {
  ...
  run = () => {
    selectedIndex = -1;
    [ids, labels, views] = buildData(1000);
    data.version++;
  },
  runLots = () => {
    selectedIndex = -1;
    [ids, labels, views] = buildData(10000);
    data.version++;
  },
  swapRows = () => {
    if (ids.length > 998) {
...
function createRowView(id, label, selected) {
  return html`<tr class="${selected ? 'danger' : false}"><td class="col-md-1">${id}</td><td class="col-md-4"><a>${label}</a></td><td class="col-md-1"><a><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></a></td><td class="col-md-6"></td></tr>`;
}
